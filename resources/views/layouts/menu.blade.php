<li class="{{ Request::is('roomAndBoards*') ? 'active' : '' }}">
    <a href="{!! route('roomAndBoards.index') !!}"><i class="fa fa-edit"></i><span>Room And Boards</span></a>
</li>

<li class="{{ Request::is('roomAndBoards*') ? 'active' : '' }}">
    <a href="{!! route('roomAndBoards.index') !!}"><i class="fa fa-edit"></i><span>Room And Boards</span></a>
</li>

<li class="{{ Request::is('roomAndBoards*') ? 'active' : '' }}">
    <a href="{!! route('roomAndBoards.index') !!}"><i class="fa fa-edit"></i><span>Room And Boards</span></a>
</li>

<li class="{{ Request::is('roomAndBoards*') ? 'active' : '' }}">
    <a href="{!! route('roomAndBoards.index') !!}"><i class="fa fa-edit"></i><span>Room And Boards</span></a>
</li>

<li class="{{ Request::is('roomAndBoards*') ? 'active' : '' }}">
    <a href="{!! route('roomAndBoards.index') !!}"><i class="fa fa-edit"></i><span>Room And Boards</span></a>
</li>

<li class="{{ Request::is('roomAndBroads*') ? 'active' : '' }}">
    <a href="{!! route('roomAndBroads.index') !!}"><i class="fa fa-edit"></i><span>Room And Broads</span></a>
</li>

<li class="{{ Request::is('roomAndBroads*') ? 'active' : '' }}">
    <a href="{!! route('roomAndBroads.index') !!}"><i class="fa fa-edit"></i><span>Room And Broads</span></a>
</li>

<li class="{{ Request::is('roomAndBroads*') ? 'active' : '' }}">
    <a href="{!! route('roomAndBroads.index') !!}"><i class="fa fa-edit"></i><span>Room And Broads</span></a>
</li>

<li class="{{ Request::is('roomAndBroads*') ? 'active' : '' }}">
    <a href="{!! route('roomAndBroads.index') !!}"><i class="fa fa-edit"></i><span>Room And Broads</span></a>
</li>

<li class="{{ Request::is('roomAndBroads*') ? 'active' : '' }}">
    <a href="{!! route('roomAndBroads.index') !!}"><i class="fa fa-edit"></i><span>Room And Broads</span></a>
</li>

<li class="{{ Request::is('roomAndBroads*') ? 'active' : '' }}">
    <a href="{!! route('roomAndBroads.index') !!}"><i class="fa fa-edit"></i><span>Room And Broads</span></a>
</li>

<li class="{{ Request::is('roomAndBroads*') ? 'active' : '' }}">
    <a href="{!! route('roomAndBroads.index') !!}"><i class="fa fa-edit"></i><span>Room And Broads</span></a>
</li>

<li class="{{ Request::is('roomAndBroads*') ? 'active' : '' }}">
    <a href="{!! route('roomAndBroads.index') !!}"><i class="fa fa-edit"></i><span>Room And Broads</span></a>
</li>

<li class="{{ Request::is('roomAndBroads*') ? 'active' : '' }}">
    <a href="{!! route('roomAndBroads.index') !!}"><i class="fa fa-edit"></i><span>Room And Broads</span></a>
</li>

<li class="{{ Request::is('roomAndBoards*') ? 'active' : '' }}">
    <a href="{!! route('roomAndBoards.index') !!}"><i class="fa fa-edit"></i><span>Room And Boards</span></a>
</li>

<li class="{{ Request::is('roomAndBoards*') ? 'active' : '' }}">
    <a href="{!! route('roomAndBoards.index') !!}"><i class="fa fa-edit"></i><span>Room And Boards</span></a>
</li>

<li class="{{ Request::is('roomAndBoards*') ? 'active' : '' }}">
    <a href="{!! route('roomAndBoards.index') !!}"><i class="fa fa-edit"></i><span>Room And Boards</span></a>
</li>

<li class="{{ Request::is('roomAndBoards*') ? 'active' : '' }}">
    <a href="{!! route('roomAndBoards.index') !!}"><i class="fa fa-edit"></i><span>Room And Boards</span></a>
</li>

<li class="{{ Request::is('roomAndBoards*') ? 'active' : '' }}">
    <a href="{!! route('roomAndBoards.index') !!}"><i class="fa fa-edit"></i><span>Room And Boards</span></a>
</li>

<li class="{{ Request::is('roomAndBoards*') ? 'active' : '' }}">
    <a href="{!! route('roomAndBoards.index') !!}"><i class="fa fa-edit"></i><span>Room And Boards</span></a>
</li>

<li class="{{ Request::is('roomAndBoards*') ? 'active' : '' }}">
    <a href="{!! route('roomAndBoards.index') !!}"><i class="fa fa-edit"></i><span>Room And Boards</span></a>
</li>

<li class="{{ Request::is('roomAndBoards*') ? 'active' : '' }}">
    <a href="{!! route('roomAndBoards.index') !!}"><i class="fa fa-edit"></i><span>Room And Boards</span></a>
</li>

<li class="{{ Request::is('roomAndBoards*') ? 'active' : '' }}">
    <a href="{!! route('roomAndBoards.index') !!}"><i class="fa fa-edit"></i><span>Room And Boards</span></a>
</li>

<li class="{{ Request::is('roomAndBoards*') ? 'active' : '' }}">
    <a href="{!! route('roomAndBoards.index') !!}"><i class="fa fa-edit"></i><span>Room And Boards</span></a>
</li>

<li class="{{ Request::is('roomAndBoards*') ? 'active' : '' }}">
    <a href="{!! route('roomAndBoards.index') !!}"><i class="fa fa-edit"></i><span>Room And Boards</span></a>
</li>

<li class="{{ Request::is('roomAndBoards*') ? 'active' : '' }}">
    <a href="{!! route('roomAndBoards.index') !!}"><i class="fa fa-edit"></i><span>Room And Boards</span></a>
</li>

<li class="{{ Request::is('roomAndBoards*') ? 'active' : '' }}">
    <a href="{!! route('roomAndBoards.index') !!}"><i class="fa fa-edit"></i><span>Room And Boards</span></a>
</li>

<li class="{{ Request::is('roomAndBoards*') ? 'active' : '' }}">
    <a href="{!! route('roomAndBoards.index') !!}"><i class="fa fa-edit"></i><span>Room And Boards</span></a>
</li>

<li class="{{ Request::is('roomAndBoards*') ? 'active' : '' }}">
    <a href="{!! route('roomAndBoards.index') !!}"><i class="fa fa-edit"></i><span>Room And Boards</span></a>
</li>

<li class="{{ Request::is('roomAndBoards*') ? 'active' : '' }}">
    <a href="{!! route('roomAndBoards.index') !!}"><i class="fa fa-edit"></i><span>Room And Boards</span></a>
</li>

<li class="{{ Request::is('roomAndBoards*') ? 'active' : '' }}">
    <a href="{!! route('roomAndBoards.index') !!}"><i class="fa fa-edit"></i><span>Room And Boards</span></a>
</li>

<li class="{{ Request::is('roomAndBoards*') ? 'active' : '' }}">
    <a href="{!! route('roomAndBoards.index') !!}"><i class="fa fa-edit"></i><span>Room And Boards</span></a>
</li>

<li class="{{ Request::is('roomAndBoards*') ? 'active' : '' }}">
    <a href="{!! route('roomAndBoards.index') !!}"><i class="fa fa-edit"></i><span>Room And Boards</span></a>
</li>

<li class="{{ Request::is('roomAndBoards*') ? 'active' : '' }}">
    <a href="{!! route('roomAndBoards.index') !!}"><i class="fa fa-edit"></i><span>Room And Boards</span></a>
</li>

<li class="{{ Request::is('roomAndBoards*') ? 'active' : '' }}">
    <a href="{!! route('roomAndBoards.index') !!}"><i class="fa fa-edit"></i><span>Room And Boards</span></a>
</li>

<li class="{{ Request::is('roomAndBoards*') ? 'active' : '' }}">
    <a href="{!! route('roomAndBoards.index') !!}"><i class="fa fa-edit"></i><span>Room And Boards</span></a>
</li>

<li class="{{ Request::is('roomAndBoards*') ? 'active' : '' }}">
    <a href="{!! route('roomAndBoards.index') !!}"><i class="fa fa-edit"></i><span>Room And Boards</span></a>
</li>

<li class="{{ Request::is('roomAndBoards*') ? 'active' : '' }}">
    <a href="{!! route('roomAndBoards.index') !!}"><i class="fa fa-edit"></i><span>Room And Boards</span></a>
</li>

<li class="{{ Request::is('roomAndBoards*') ? 'active' : '' }}">
    <a href="{!! route('roomAndBoards.index') !!}"><i class="fa fa-edit"></i><span>Room And Boards</span></a>
</li>

<li class="{{ Request::is('roleChangeStatuses*') ? 'active' : '' }}">
    <a href="{!! route('roleChangeStatuses.index') !!}"><i class="fa fa-edit"></i><span>Role Change Statuses</span></a>
</li>

<li class="{{ Request::is('levelRoleStatuses*') ? 'active' : '' }}">
    <a href="{!! route('levelRoleStatuses.index') !!}"><i class="fa fa-edit"></i><span>Level Role Statuses</span></a>
</li>

<li class="{{ Request::is('transactionRoleStatuses*') ? 'active' : '' }}">
    <a href="{!! route('transactionRoleStatuses.index') !!}"><i class="fa fa-edit"></i><span>Transaction Role Statuses</span></a>
</li>

<li class="{{ Request::is('claimWordSheets*') ? 'active' : '' }}">
    <a href="{!! route('claimWordSheets.index') !!}"><i class="fa fa-edit"></i><span>Claim Word Sheets</span></a>
</li>

<li class="{{ Request::is('providers*') ? 'active' : '' }}">
    <a href="{!! route('providers.index') !!}"><i class="fa fa-edit"></i><span>Providers</span></a>
</li>

